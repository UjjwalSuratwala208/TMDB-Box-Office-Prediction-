...
# print the sample starting records of training file
top_train_records = train.head(10)
print('Starting Sample Records of train.csv file:- \n')
print(top_train_records)

# print the sample starting records of testing file
top_test_records = test.head(10)
print('\n Starting Sample Records of test.csv file:- \n')
print(top_test_records)

# display the statistical summary of the data
train_summary = train.describe()
print('\n Satisitical summary of train.csv file:-')
print(train_summary)

# display the statistical summary of the data
test_summary = test.describe()
print('\n Satisitical summary of test.csv file:-')
print(test_summary)

# display the infromation of train.csv data file
data_train_info = train.info()
print('\n Data_information of train.csv file:-')
print(data_train_info)

# display the infromation of test.csv data file
data_test_info = test.info()
print('\n Data_information of test.csv file:-')
print(data_test_info)

# count the total no of null value according to the particlr field
print("\n count of null value for particular field:- ", train.isna().sum())

print(" \n count the value movie according to the status in train file:-  ",train['status'].value_counts())

print(" \n count the value movie according to the status in test file:-  ",test['status'].value_counts())

# summary of revenue field
print("\n summary of revenue field of train file:-",train.revenue.describe())

# summary of budget field
print("\n summary of budget field of train file:-",train.budget.describe())

# scatterplot plot between budget and revenue
sns.jointplot(x="budget", y="revenue", data=train, height=10, ratio=6, color="b")
plt.show()

#scatterplot plot between popularity and revenue
sns.jointplot(x="popularity", y="revenue", data=train, height=10, ratio=6, color="b")
plt.show()

#scatterplot plot between runtime and revenue
sns.jointplot(x="runtime", y="revenue", data=train, height=10, ratio=6, color="b")
plt.show()

#Distance plot according the revenue
sns.distplot(train.revenue)
plt.show()

#distance plot using revenue filed log value
train['logRevenue'] = np.log1p(train['revenue'])
sns.distplot(train['logRevenue'] )

# getting the year. month and day from the release_date
...
#bar chart for  howmany movie  hadr eleased in a particulr year
...
#bar chart for  howmany movie  had released in a particulr month
plt.figure(figsize=(20,10))
...
#bar chart for  howmany movie  had released on a particulr day
plt.figure(figsize=(20,10))
...
#bar chart for  howmany movie  had released in a throughout each day of a week
plt.figure(figsize=(20,10))
...
#timeline graph of movies average revenue  throughout a month
train['meanRevenueByMonth'] = train.groupby("release_month")["revenue"].aggregate('mean')
train['meanRevenueByMonth'].plot(figsize=(15,12),color="b")
plt.xlabel("Release Month")
plt.ylabel("Revenue")
plt.title("Movie Mean Revenue Release Month",fontsize=20)
plt.show()

#timeline graph of movies average revenue  throughout a qauter
train['meanRevenueByQuarter'] = train.groupby("release_quarter")["revenue"].aggregate('mean')
train['meanRevenueByQuarter'].plot(figsize=(15,12),color="b")
...
# timeline graph of movies average runtime  throughout a year
train['meanruntimeByYear'] = train.groupby("release_year")["runtime"].aggregate('mean')
train['meanruntimeByYear'].plot(figsize=(15,12),color="b")
plt.xticks(np.arange(1920,2018,4))
plt.xlabel("Release Year")
plt.ylabel("Runtime")
...
#timeline graph of movies average popularity throughout a year
train['meanPopularityByYear'] = train.groupby("release_year")["popularity"].aggregate('mean')
train['meanPopularityByYear'].plot(figsize=(15,12),color="g")
plt.xticks(np.arange(1920,2018,4))
plt.xlabel("Release Year")
...
#timeline graph of movies average budget throughout a year
...
# conunt of the movie according to the geners in train file
...
print(" \n count of movies according to the geners in train file- \n \t")
...
# conunt of the movie according to the geners in test file
test = test
...
print(" \n count of movies according to the geners in test file- \n \t")
...
#Bargraph for couting the total no of language in a  training dataset
plt.figure(figsize=(20,15))
sns.countplot(train['original_language'].sort_values())
plt.title("Original Language Count",fontsize=20)
plt.show()

# bar chart to count how many movies has homepage
...
#scatterplot or correlation plot between movie with and without homepage and revenue
sns.catplot(x="has_homepage", y="revenue", data=train)
plt.title('Revenue of movies with and without homepage')

#scatterplot or correlation plot between movie with and without tagline and revenue
train['isTaglineNA'] = 0
...
#scatterplot or correlation plot between movie with single and multiple title and revenue
...
#scatterplot or correlation plot between movie with language as english and not english  and revenue
...
#adding a new file for getting better EDA
trainAdditionalFeatures = pd.read_csv("..\\dataset\\TrainAdditionalFeatures.csv")
testAdditionalFeatures = pd.read_csv("..\\dataset\\TestAdditionalFeatures.csv")

train = pd.merge(train, trainAdditionalFeatures, how='left', on=['imdb_id'])
test = pd.merge(test, testAdditionalFeatures, how='left', on=['imdb_id'])

#printing the missing value of ratings and totalvotes from train and test file
...
# filling the null value of a rating filed with a value of 1.5 and total votes value using 6
...
#graph of average mean of movie by total votes by year
train['meantotalVotesByYear'] = train.groupby("release_year")["totalVotes"].aggregate('mean')
train['meantotalVotesByYear'].plot(figsize=(15,10),color="r")
plt.xticks(np.arange(1920,2018,4))
plt.xlabel("Release Year")
plt.ylabel("Rating")
...
#graph of average mean of movie by total votes by ratings
...
# prepating the data for analysis
def prepare(df):
    global json_cols
    global train_dict
...
train = pd.merge(train, pd.read_csv("..\\dataset\\TrainAdditionalFeatures.csv"),
                 how='left', on=['imdb_id'])
test = pd.merge(test, pd.read_csv("..\\dataset\\TestAdditionalFeatures.csv"), how='left',
                on=['imdb_id'])

additionalTrainData = pd.read_csv("..\\dataset\\additionalTrainData.csv")
...
#applytind the kfold cross validation on the dataset
...
#perfroming the catboost regression
...
#displaying the result of lgbm, catboost, xgboost model
result_dict = dict()
...
#write and generate the output in the csv file
ub = pd.read_csv("..\\dataset\\sample_submission.csv")
df_sub = pd.DataFrame()
df_sub['id'] = sub['id']
